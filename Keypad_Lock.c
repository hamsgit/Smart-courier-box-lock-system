...
// System Structure
typedef struct {
    char current_password[6];
    char password_history[5][6];
    char input_buffer[6];
    int input_pos;
    menu_state_t state;
    bool authenticated;
...
// Password Management
static void load_password(void) {
    size_t required_size = sizeof(lock_system.current_password);
    esp_err_t err = nvs_get_blob(nvs_handler, "current_pwd", lock_system.current_password, &required_size);
    
    if (err == ESP_ERR_NVS_NOT_FOUND || required_size == 0) {
        strcpy(lock_system.current_password, "1234");
        save_password();
    }

    for (int i = 0; i < 5; i++) {
        char key[12];  // Increased buffer size to prevent truncation
        snprintf(key, sizeof(key), "pwd_hist_%d", i);
        nvs_get_blob(nvs_handler, key, lock_system.password_history[i], &required_size);
    }
}

static void save_password(void) {
    for (int i = 4; i > 0; i--) {
        strcpy(lock_system.password_history[i], lock_system.password_history[i-1]);
    }
    strcpy(lock_system.password_history[0], lock_system.current_password);

    nvs_set_blob(nvs_handler, "current_pwd", lock_system.current_password, sizeof(lock_system.current_password));
    
    for (int i = 0; i < 5; i++) {
        char key[12];  // Increased buffer size to prevent truncation
        snprintf(key, sizeof(key), "pwd_hist_%d", i);
        nvs_set_blob(nvs_handler, key, lock_system.password_history[i], sizeof(lock_system.password_history[i]));
    }
...
static void display_menu(menu_state_t state) {
    ...
    switch(state) {
        ...
        case SETTINGS_MENU:
            ...
            lcd_set_cursor(0, 1);
            lcd_print_str("A:Change B:Back");
            break;
            
        case CHANGE_PASSWORD:
            lcd_print_str("New Password:");
            lcd_set_cursor(0, 1);
            for (int i = 0; i < lock_system.input_pos; i++) {
                lcd_print_str("*");
            }
        ...
        case VIEW_HISTORY:
            lcd_print_str("Password History:");
            lcd_set_cursor(0, 1);
            lcd_print_str("A:Back");
            break;
...
static void handle_keypress(char key) {
    switch(lock_system.state) {
        ...
        case UNLOCK_MODE:
            if (key == '#') {
                if (strcmp(lock_system.input_buffer, lock_system.current_password) == 0) {
                    control_lock(true);
                    lcd_send_cmd(LCD_CLEAR_DISPLAY);
        ...
        case SETTINGS_MENU:
            if (key == 'A') {
                lock_system.state = CHANGE_PASSWORD;
                memset(lock_system.input_buffer, 0, sizeof(lock_system.input_buffer));
                lock_system.input_pos = 0;
            } else if (key == 'B') {
                lock_system.state = MAIN_MENU;
        ...
        case CHANGE_PASSWORD:
            if (key == '#') {
                if (lock_system.input_pos >= 4) {
                    strcpy(lock_system.current_password, lock_system.input_buffer);
                    save_password();
                    lcd_send_cmd(LCD_CLEAR_DISPLAY);
                    lcd_print_str("Password Changed!");
                    vTaskDelay(pdMS_TO_TICKS(2000));
                } else {
                    lcd_send_cmd(LCD_CLEAR_DISPLAY);
